...
public class GeometricMean extends Task implements Runnable{
	...
	public GeometricMean(double number1, Pipe<ShareData> pipe) {
		super(number1, pipe);
		num1 = number1;
		toControl = pipe;
	...
	public void run() {
		...
		try {
			probability = stat.geometricExpectedValue(num1);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
			
		ShareData ret = new DoubleAndString(probability, "Geometric Mean");
		toControl.put(ret);
	}
