...
public class BinomialSD extends Task implements Runnable{
	...
	public BinomialSD(double number1, double number2, Pipe<ShareData> pipe) {
	...
	public void run() {
		...
		try {
			probability = stat.binomialStdDev(num1, num2);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
			
		ShareData ret = new DoubleAndString(probability, "Binomial Std Dev");
		toControl.put(ret);
	}
