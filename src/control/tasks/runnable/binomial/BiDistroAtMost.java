...
public class BiDistroAtMost extends Task implements Runnable{
	...
	public BiDistroAtMost(double[] number, Pipe<ShareData> pipe) {
		super(number, pipe);
		data = number;
		toControl = pipe;
	...
	public void run() {
		...
		try {
			probability = stat.binomialDistributionAtMost(data[0], data[1], data[2]);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
			
		ShareData ret = new DoubleAndString(probability, "biDistroAtMost");
		toControl.put(ret);
	}
