...
public class BinomialVariance extends Task implements Runnable{
	...
	public BinomialVariance(double number1, double number2, Pipe<ShareData> pipe) {
	...
	public void run() {
		...
		try {
			probability = stat.binomialVariance(num1, num2);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
			
		ShareData ret = new DoubleAndString(probability, "Binomial Variance");
		toControl.put(ret);
	}
